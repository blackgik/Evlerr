...
const ErrorHandler = function (error, req, res, next) {
    if (error.name === "VobbioError" || error.isOperational) {
        return res.status(error.statusCode).send((0, appResponse_1.default)(error.message, null, false));
    }
    if (error instanceof mongooseValidationError) {
        const errorMessages = Object.values(error.errors).map((e) => e.message);
        return res
            .status(400)
            .send((0, appResponse_1.default)("validation error occurred check your inputs for corrections", null, errorMessages));
    }
    if (error.hasOwnProperty("name") && error.name === "MongoError") {
        const data = error && error.errmsg ? error.errmsg : null;
        return res.status(400).send((0, appResponse_1.default)("the entry already exist", data, false));
    }
    if (errorNames.includes(error.name)) {
        if (error.name === "TokenExpiredError") {
            return res
                .status(400)
                .send((0, appResponse_1.default)("Token has expired, Request a reset password again", null, false));
        }
